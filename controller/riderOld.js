    ...
    if (!errors.isEmpty()) {
        ...
        try {
            ...
            Rider.findOne({
                ...
                .then(rider => {
                    if (rider) {
                        var JWTRdier = rider.email.value;
                            ...
                            Fleet.findOne({
                                ...
                                .then(fleet => {
                                    bcrypt.compare(req.body.password, fleet.password.hash, function (err, result) {
                                        if (result) {
                                            if (rider.status.account != "Pending") {
                                                if (rider.status.account == "Approved") {
                                                    jwt.sign({
                                                        email: rider.email.value,
                                                        id: rider._id
                                                    }, process.env.SECRET, (err, token) => {
                                                        if (err) {
                                                            ...
                                                            let salt = crypto.randomBytes(16).toString('base64');
                                                            let hash = crypto.createHmac('sha512', salt).update(rider.id).digest("base64");
                                                            let b = Buffer.from(hash);
                                                            let refresh_token = b.toString('base64');
                                                            res.status(201).json({
                                                                riderData: {
                                ...
                                .catch(error => {
                                    res.status(407).json({
                                        message: "Incorrect Fleet Owner"
                                    })
                                })
                        } else {
                            bcrypt.compare(req.body.password, rider.password.hash, function (err, result) {
                                if (result) {
